...
class OrderManager {
        ...
        if (saved) {
            try {
                ...
                if (auth.expires > Date.now()) {
                    ...
                    // Продлеваем срок хранения при каждом входе если стоит remember
                    if (auth.remember) {
                    ...
                    // Если срок истек - очищаем
                    localStorage.removeItem('xplay_auth');
                    console.log('Срок авторизации истек');
                }
            } catch (e) {
    ...
    refreshAuthExpiry() {
        ...
        if (saved) {
            try {
                ...
                console.log('Авторизация продлена до:', new Date(auth.expires).toLocaleString());
            } catch (e) {
    ...
    async checkAuthAndAutoLogin() {
        ...
        try {
            ...
            // Если есть сохраненная авторизация и флаг remember = true
            if (auth.remember) {
                console.log('Автоматический вход с сохраненными данными для:', auth.user);
                ...
                // Если remember = false, очищаем авторизацию при обновлении страницы
                console.log('Сессионная авторизация, требуется повторный вход');
                localStorage.removeItem('xplay_auth');
                return false;
            }
        } catch (e) {
        ...
        try {
            ...
            if (data.success) {
                ...
                const authData = {
                    ...
                    remember: remember // Сохраняем флаг "запомнить"
                };
                
                localStorage.setItem('xplay_auth', JSON.stringify(authData));
                
                console.log('Авторизация сохранена:', {
                    user: this.currentUser,
    ...
    async init() {
        ...
        if (isAuth) {
            // Если авторизован - загружаем данные и показываем интерфейс
            this.updateUIForAuth();
    ...
    async confirmDeleteOrder() {
        ...
        console.log('ID заказа для удаления:', this.currentOrderId);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (autoLoggedIn) {
        // Если авто-вход успешен - обновляем UI
        orderManager.updateUIForAuth();
        
        // Загружаем данные с прогрессом
        await orderManager.loadOrdersWithProgress();
        
        // Показываем дашборд
        orderManager.showDashboard();
    } else {
        // Если нет - обычная инициализация

...
class OrderManager {
            ...
            if (cached) {
                try {
                    ...
                    if (data.orders && Array.isArray(data.orders)) {
                        ...
                        if (!silent) {
                            this.showNotification(`📦 Загружено ${this.orders.length} заказов`, 'info', 2000);
                        }
                    }
                } catch (e) {}
            }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (!this.loadFromCache()) {
                    this.showNotification('Не удалось загрузить данные', 'warning');
                }
            }
        } catch (error) {
            console.error('Ошибка загрузки:', error);
    ...
    findNewlyCreatedOrder(orderData) {
        ...
        const foundOrder = sortedOrders.find(order => {
            ...
            return orderPhone === searchPhone && orderName === searchName && isRecent;
        });
    ...
    async createOrder(orderData) {
        ...
        try {
            ...
            this.showProgressBar('Обновление списка...', 80);
            ...
            <body>
                <div class="contract">
                    ...
                    <div class="conditions">
                        ...
                        1. Исполнитель обязуется провести диагностику и ремонт устройства.<br>
                        2. Срок и стоимость могут быть изменены при обнаружении скрытых неисправностей.<br>
                        3. На выполненные работы предоставляется гарантия.<br>
                        4. При отказе от ремонта оплачивается диагностика: 300-800 руб.
                    </div>

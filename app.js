...
const FETCH_TIMEOUT = 60000; // 60 секунд таймаут для медленного Google Apps Script
...
class OrderManager {
    ...
    async login(login, password, remember = false) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Затем синхронизируем с сервером в фоне (не блокируем вход!)
            ...
            if (error.message && error.message.includes('Превышено время')) {
                this.showNotification('⏳ Сервер медленно отвечает. Попробуйте ещё раз или обновите страницу.', 'warning', 7000);
            } else {
    ...
    async loadOrdersInBackground() {
        ...
        try {
            ...
            console.log('Фоновое обновление не удалось, повтор через 5 сек:', error.message);
            setTimeout(() => {
                this.loadOrdersWithTimeout(true).catch(err => {
                    console.log('Повторная попытка тоже не удалась:', err.message);
                });
            }, 5000);
        } finally {
    ...
    async loadOrdersWithTimeout(silent = true) {
        ...
        try {
            ...
            if (!silent || this.orders.length === 0) {
                if (this.orders.length === 0) {
                    this.showNotification('⚠️ Не удалось загрузить заказы. Проверьте соединение.', 'warning', 7000);
                }
            }
    ...
    async createOrder(orderData) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Если это дубликат — показываем другое сообщение
                if (data.isDuplicate) {
                    setTimeout(() => {
                        this.hideProgressBar();
        ...
        try {
            ...
            if (data.success) {
                ...
                if (orderIndex !== -1) {
                    ...
                    if (updates.finalPrice !== undefined) this.orders[orderIndex].finalprice = updates.finalPrice;
                ...
                this.showNotification('❌ Ошибка сервера: ' + (data.error || 'Неизвестная ошибка'), 'danger');
                return false;
            }
        } catch (error) {
            ...
            if (error.message && error.message.includes('Превышено время')) {
                try {
                    ...
                    if (order) {
                        let updated = true;
                        if (updates.status && order.status !== updates.status) updated = false;
                        if (updates.finalPrice !== undefined && order.finalprice != updates.finalPrice) updated = false;
                        
                        if (updated) {
                            this.showNotification('✅ Заказ успешно обновлен (с задержкой сервера)', 'success');
                            this.refreshCurrentView();
                            return true;
                        }
                    }
                } catch (retryError) {
                    console.error('Ошибка проверки:', retryError);
                }
                
                this.showNotification('⚠️ Сервер не отвечает. Попробуйте обновить страницу и проверить заказ.', 'warning');
            } else {
    ...
    async closeOrder(id, finalPrice) {
        const orderIndex = this.orders.findIndex(o => o.id === id);
        if (orderIndex !== -1) {
            this.orders[orderIndex].status = 'Выдан';
            this.orders[orderIndex].finalprice = finalPrice;
            this.orders[orderIndex].completiondate = new Date().toLocaleString('ru-RU', { timeZone: 'UTC' });
            this.saveToCache();
        }
        ...
        if (!result && orderIndex !== -1) {
            this.showNotification('⚠️ Локальные изменения сохранены. Синхронизация будет выполнена автоматически.', 'warning');
            this.refreshCurrentView();
    ...
    async restoreOrder(id) {
        const orderIndex = this.orders.findIndex(o => o.id === id);
        if (orderIndex !== -1) {
            this.orders[orderIndex].status = 'Принят';
            this.orders[orderIndex].finalprice = '';
            this.orders[orderIndex].completiondate = '';
            this.saveToCache();
        }
    ...
    getStatistics() {
        ...
        this.orders.forEach(o => {
            const dateStr = o.createdat || o.acceptancedate;
    ...
    async confirmCloseOrder() {
        ...
        if (!finalPrice && finalPrice !== '0') {
            this.showNotification('Введите итоговую стоимость (или 0)', 'warning');
            return;
        }
        
        const closeModal = bootstrap.Modal.getInstance(document.getElementById('closeOrderModal'));
        const viewModal = bootstrap.Modal.getInstance(document.getElementById('viewOrderModal'));
    ...
    renderMonthlyStats(monthly) {
        const months = Object.entries(monthly).reverse();
        if (months.length === 0) {
...
// Перезагрузка приложения при клике на логотип
function reloadApp() {
    location.reload();
}
